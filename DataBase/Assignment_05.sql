...
HAVING COUNT(account_id) = (
							SELECT MAX(eig_count)
                            FROM (
									SELECT COUNT(account_id) AS eig_count 
                                    FROM account
                                    LEFT JOIN group_account USING(account_id)
                                    GROUP BY account_id) as count_eig);
                                    
-- CTE
CREATE OR REPLACE VIEW view_02_1 AS
WITH num_of_e AS (
	SELECT account.*, COUNT(account_id) AS counting_e
    FROM account
    LEFT JOIN group_account USING(account_id)
    GROUP BY account_id
)
...
HAVING COUNT(account_id) =(
							SELECT MAX(e_count)
							FROM (
								 SELECT COUNT(account_id) AS e_count
								 FROM account
								 RIGHT JOIN department USING(department_id)
								 GROUP BY department_id) as count_e);
                                 
-- CTE
WITH count_e AS (
				SELECT department.department_name, COUNT(account_id) AS e_count
				FROM account
				RIGHT JOIN department USING(department_id)
				GROUP BY department_id
)
